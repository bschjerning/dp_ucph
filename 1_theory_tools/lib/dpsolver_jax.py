...
class dpsolver():
    ...
    def vfi_T(model, T=100, callback=None):
        ...
        V=jnp.zeros((model.n_x, T+1)) # on first iteration assume consuming everything
        policy = jnp.zeros((model.n_x, model.n_choices, T+1))  # Stores policy functions (p1, p2, ..., pn_choices)'''
    ...
    def iterinfo(iter,model,V1,c=None, V0=0):
        print('iter=', iter, '||V1-V0||', jnp.max(abs(V1-V0)))
