...
def EGM(sol,z_plus,p,t,par): 
    ...
    ### SOLUTION ###
    # 1. get expected marginal utility of consumption and expected value
    if z_plus == 1:     # retired
        EV_raw, EMU_plus = retired(sol,z_plus,p,t,par)
    else:               # working
        EV_raw, EMU_plus = working(sol,z_plus,p,t,par)
    # 2. get raw grid for cash-in-hand, value and consumption using standard egm
...
def retired(sol,z_plus,p,t,par):
    ...
    # 3. value
    EV_raw = tools.interp_2d_vec(par.grid_m,par.grid_p,sol.v[t+1,z_plus], m_plus, p_plus)
    # 4. consumption
    c_plus = tools.interp_2d_vec(par.grid_m,par.grid_p,sol.c[t+1,z_plus], m_plus, p_plus)
    # 5. marginal utility
    MU_plus = marg_util(c_plus,par)
    # 6. expected average marginal utility
    EMU_plus = MU_plus * w
    ### SOLUTION ###
...
def working(sol,z_plus,p,t,par):
    ...
    # 3. allocate value, consumption, marg_util
