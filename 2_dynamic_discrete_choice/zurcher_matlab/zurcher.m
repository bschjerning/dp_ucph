...
methods (Static)
  ...
  function [V1, pk, dbellman_dV]=bellman(V0, mp, u, P)
    ...
    % Inputs and outputs: see bellman_iv and bellman_ev
  ...
  function [V1, pk, dBellman_dV]=bellman_iv(V0, mp, u, P)
    % zurcher.bellman_iv:    Procedure to compute bellman integrated Bellman equation
    %
    % Inputs
  ...
  function [ev1, pk, dbellman_dev]=bellman_ev(ev, mp, u, P)
    ...
    vK= u(:,1) + mp.beta*ev;      % Value off keep
    vR= u(:,2) + mp.beta*ev(1);   % Value of replacing
  ...
  function [f,g,h]=ll(data, mp, theta)
    ...
    % Evaluate choice probabilities evaluated at observed state 
    px_j=[pk(data.x) 1-pk(data.x)];  % [keep replace]
    ...
    if nargout >=2;  % compute scores (use chain rule - three steps)
      ...
      % ...then wrt. transition parameters
      if n_P>0        
        for iP=1:n_P;
      ...
      % gradient (of negative of mean log likelihood)
      g=mean(-score,1);
    end
