...
x_a=0.52;
x_b=0.22;
T=100 % number of times game is played
...
nbar=20;
subplot(3,2,2*esr-1), hist(theta_hat(:,1),nbar); title(sprintf('True alpha=%1.1f', alpha));  %xlim([4 6]);
subplot(3,2,2*esr), hist(theta_hat(:,2),nbar); title(sprintf('True beta=%1.1f', beta));  %xlim([-13 -9]);
